...
func main() {
	...
	v2 := router.Group("/v2")
	{
		...
		SplitTransaction := v2.Group("/split-transaction")
		{
			...
			SplitTransaction.POST("/get-splits", controllers.GetSplitTransactions)
			SplitTransaction.POST("/splits", controllers.GetSplitTransactionsString)
